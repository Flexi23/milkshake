/**
 * milkshake -- WebGL Milkdrop-esque visualisation (port of projectM) Copyright
 * (C)2011 Matt Gattis and contributors
 * 
 ...
 * 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA See 'LICENSE.txt'
...
var Music = Class
		.extend({
			...
			addPCM : function(left, right) {
				...
				if (this.numsamples == left.length
						&& this.numsamples.right == right.length)
			...
			detectFromSamples : function() {
				...
				for (i = 0; i < 16; i++) {
					...
					for (j = linear * 2; j < (linear + 8 + i) * 2; j++) {
						this.beat_instant[i] += ((this.pcmdataL[j] * this.pcmdataL[j]) + (this.pcmdataR[j] * this.pcmdataR[j]))
								/ (8 + i);
					...
					this.beat_val[i] = this.beat_instant[i]
							/ this.beat_history[i];
				}
			...
			getPCM : function(PCMdata, samples, channel, freq, smoothing) {
				...
				for ( var i = 1; i < samples; i++)
					PCMdata[i] = (1 - smoothing)
							* PCMd[this.numsamples - 1 - i] + smoothing
							* PCMdata[i - 1];
				if (freq) {
					throw Error("there's a buffer of length: " + this.numsamples + " ("+samples+")"); // 512 numsamples, 41 samples
